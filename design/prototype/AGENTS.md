...
- The selected visual direction is the warm family companion concept: ivory background, coral primary actions, friendly rounded cards, large Korean type, and calm trust cues.
- Keep exactly three primary bottom destinations: `채팅`, `채팅 목록`, and `설정`, each with an icon and visible Korean label.
- First-run help is a four-step, skippable coach-mark flow with previous/next controls. It teaches text/voice input, half-duplex interruption, search privacy, and text/voice preferences without requesting permissions.
- Senior defaults are high contrast, at least 18sp body copy, at least 48dp targets (56dp for primary actions), explicit Korean state labels, and no color-only meaning.
- Monetization UI must preserve a useful free offline core. The settings screen may compare `무료` and an inexpensive `내새끼 플러스`, but must label unimplemented prices and payment actions as planned/preview-only, explain online data transmission, and never use urgency or senior-hostile dark patterns.
