...
Android 앱에는 채팅·음성 하프듀플렉스·한국어 음성팩 준비·Gemma 4 화면 비전·모델 설치·암호화 기록과 개인 기억·대표 출처 링크가 있는 검색·확인형 앱 동작·구독 미리보기 UI가 연결되어 있습니다. 모델 가중치, 결제, 검색 제공자 키와 클라우드 모델 라우팅은 저장소에 포함하지 않습니다. E2B의 텍스트·화면 비전과 Supertonic 합성은 AVD에서 실제 모델로 검증했고, E4B GPU·한국어 STT 자연스러움·다양한 실제 앱 화면 정확도·열/배터리는 실기기 QA 대상입니다.
...
- 화면 공유는 Android의 매회 동의를 거쳐 시작하고, 공유 중임을 계속 표시합니다. `작은 창`을 선택하면 다른 앱을 그대로 보며 마이크로 질문할 수 있습니다. 현재 프레임은 기기 안의 Gemma 4 비전 입력과 작은 글자 보강용 OCR에만 쓰며 파일로 저장하거나 인터넷으로 보내지 않습니다. 앱·알림·시스템 어느 경로로 중지해도 캡처 자원과 KV 캐시를 지우고, 화면을 본 대화가 이후 추론에 다시 들어가지 않도록 암호화 설정에 문맥 경계만 남깁니다.
