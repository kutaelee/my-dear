...
- The internet switch is fixed above the scrollable conversation so it remains visible and tappable at 200% system font. The settings text-size control is a real switch without a navigation arrow. Eight Compose journeys pass at both 1.0× and 2.0× font scales.
- A pinned real Gemma 4 E2B quality regression passed both reported prompts: generic blanket folding produced a direct actionable answer without repeating the clarification, and the current-president answer used supplied internet evidence with `[자료 1]` while excluding the reported fabricated name. Output is capped at 256 tokens — PASS on the CPU AVD.
