...
| Screen sharing | Per-session system consent, persistent sharing indicator, immediate stop, 0 persisted frames/OCR text, and 0 screen-derived tool executions |
...
- The internet switch now lives only in Settings, remains tappable at 200% system font, and is no longer allowed to crowd the conversation. The settings text-size control is a real switch without a navigation arrow. Ten Compose journeys pass at both 1.0× and 2.0× font scales.
...
- Preview 5 removes the internet control from the chat screen and keeps the default-on opt-out only in Settings. AVD Compose journeys confirm the switch works and does not overlap the conversation or navigation — PASS.
- Preview 6 reconciles a completed model install immediately, after process restoration, and again before the first answer, so no unrelated settings toggle is needed. Public relation questions such as `리센느 리더는?` and `미스트롯의 우승자는?` route to Korean Wikipedia, read the requested fact from the relevant page, produce a direct answer, and expose one human-readable article link. Role words without a named public subject fail closed, and short follow-ups never append or transmit prior conversation text. Weather provenance now opens the provider homepage instead of a raw JSON API URL and formats timestamps in Korean. Unit policy tests, five live public-search gateway tests, and end-to-end Compose answer/source tests cover the regression. Evidence: `preview6-readable-search-source.png`, `preview6-search-source-comparison.png`.
- Preview 7 moves LiteRT initialization off the UI thread, renders a first-answer progress bubble before tokens arrive, stops auto-follow as soon as the user manually scrolls, and offers a visible jump-to-latest control. Default answers are direct and bounded while the output ceiling increases from 256 to 512 tokens to avoid mid-sentence truncation. Explicit product-link follow-ups preserve public product context on-device and add a user-facing Naver search shortcut; private context fails closed. Voice mode has a persistent stop control, polls a scheduled Korean offline model download, and offers an explicit-consent system speech fallback when a device lacks Korean on-device recognition.
- Preview 8 treats empty or failed Android language-support reports as unresolved instead of falsely ready, shows Korean model preparation continuously, caps offline recognition model retries at one, and keeps the system-speech recovery action fixed above the composer. Generic product follow-ups now merge their qualifier with the prior category (`가성비 제품으로 링크줘` after a diffuser request becomes `가성비 방향제`) rather than searching for `가성비 제품으로`. Product-type labels are no longer presented as verified product names.
- Active screen sharing offers an Android picture-in-picture microphone window so the target app stays visible while the user asks. This avoids unrestricted overlay permission and prevents returning to the full chat UI from replacing the target screen. AVD verifies entry and layout; Korean voice continuity and answer usefulness over third-party apps remain physical-device gates.
- API 36 MediaProjection was accepted through the real system dialog, the required media-projection foreground service remained active, and both the in-app banner and notification exposed a stop action. The pinned real Gemma 4 E2B received actual AVD screenshots as `ImageBytes` with OCR text deliberately omitted: it identified the visible bottom-right `설정` tab and, with 내새끼 in picture-in-picture over Android Settings, selected `Network & internet` for a Wi-Fi question — PASS for end-to-end vision, not merely OCR. Korean OCR remains bundled only as a small-text aid. Prompt tests bound OCR text to 6,000 characters and mark it as untrusted data. Privacy-boundary unit tests verify that the latest of multiple deletion/screen boundaries wins and that a missing boundary fails closed, preventing a stopped screen session from re-entering later inference. Real-device usefulness, latency, rotation and secure-surface behavior remain QA gates. Evidence: `preview5-screen-share-active.png`, `preview5-pip-over-settings.png`.
- API 36 Korean STT support probing and download-request paths return without crashing. Unsupported devices now receive a direct Korean voice-settings action instead of the terminal “model unavailable” message — PASS for recovery UX; OEM Korean recognition remains a physical-device gate.
...
ARM CPU/GPU/NPU performance, PSS/LMK, thermal and battery, OEM Korean STT, real microphone endpointing, Supertonic naturalness, shared-screen Gemma vision/OCR accuracy across real apps, rotation while sharing, protected/blank surfaces, speaker echo, Bluetooth/hearing-aid routes, audio focus during calls, and external intent behavior are explicitly not certified by AVD.
