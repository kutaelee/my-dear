...
- Five senior UI journeys plus encrypted-at-rest round trip and confirmation-only phone action — PASS.
- The same five senior UI journeys passed with the real Android system font scale preserved at 2.0 and the app's default 1.15× senior multiplier. Fixed-height controls were changed to minimum heights and the header adapts at very large text. The primary action remains reachable by the scrollable content; a final screenshot is stored as `docs/qa/avd/final-chat-home-true-font-200.png` — PASS for the five tested journeys.
